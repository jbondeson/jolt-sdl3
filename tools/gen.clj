...
(defn c->clj
  "SDL_CreateWindowAndRenderer -> create-window-and-renderer; windowID -> window-id."
  [s]
  (let [s (if (str/starts-with? s "SDL_") (subs s 4) s)
...
(defn parse-functions [text]
  (for [[_ ret name params] (re-seq #"extern SDL_DECLSPEC (.+?)\s*SDLCALL (SDL_\w+)\s*\(([^)]*)\)\s*;" text)]
...
(defn parse-defines
  ...
  [comment-stripped]
  (into {} (for [[_ name value] (re-seq #"(?m)^\s*#\s*define\s+(SDL\w*)[ \t]+([^\n]*)$" comment-stripped)
...
(defn run-c-oracle [include-dir int-names string-names structs tmp-dir]
  (let [lines (concat
               ["#include <SDL3/SDL.h>" "#include <stdio.h>" "#include <stddef.h>" "int main(void){"]
...
(defn emit-module [types core-names module cheader fns structs layouts]
...
(defn emit-consts [enum-maps define-maps]
  (str header
...
(defn -main [& [include-dir root]]
  (let [include-dir (or include-dir
        ...
        root (or root ".")
        hdir (io/file include-dir "SDL3")
        headers (->> (.listFiles hdir) (map #(.getName %))
        ...
        groups (for [[prefix var] define-groups]
                 (let [names (sort (filter #(and (str/starts-with? % prefix) (not (enum-members %))) (keys defines)))]
                   {:var var :prefix prefix :names names
                    :kind (if (#{"SDL_HINT_" "SDL_PROP_"} prefix) :string :int)}))
        singles (filter defines define-singles)
        int-names (distinct (concat enum-members (mapcat :names (filter #(= :int (:kind %)) groups)) singles))
        string-names (distinct (mapcat :names (filter #(= :string (:kind %)) groups)))
        ;; structs (raw) — needed by the oracle for offsets
        structs-by-header (into {} (for [h headers] [h (vec (parse-structs (get clean h)))]))
        all-structs (vec (mapcat structs-by-header headers))
        _ (println "headers:" (count headers) "enums:" (count enums) "defines:" (count defines) "structs parsed:" (count all-structs))
        oracle (run-c-oracle include-dir int-names string-names
        ...
        ;; functions
        fns-by-header (into {} (for [h headers] [h (vec (parse-functions (get clean h)))]))
        declared (reduce + (map #(count (re-seq #"extern SDL_DECLSPEC" (get texts %))) headers))
        parsed (reduce + (map count (vals fns-by-header)))
        _ (println "functions declared:" declared "parsed:" parsed)
        _ (let [want (set (mapcat #(map second (re-seq #"SDLCALL (SDL_\w+)\s*\(" (get clean %))) headers))
                got (set (map :c (mapcat val fns-by-header)))]
            (doseq [n (sort (remove got want))] (println "  unparsed decl:" n)))
        ;; emit modules
        results (vec (for [h headers
                      :let [fns (get fns-by-header h) structs (get structs-by-header h)]
                      :when (or (seq fns) (some #(get layouts (:c %)) structs))]
                  (let [module (header-module h)
                        r (emit-module types core-names module h fns structs layouts)
                        path (str root "/src/sdl3/raw/" module ".clj")]
                    (io/make-parents path)
                    (spit path (:code r))
                    (assoc r :module module))))
        all-skipped (mapcat :skipped results)
        layout-vars (mapcat :layout-vars results)
        ;; consts
        ...
        define-maps (concat
                     ...
                     [{:var "misc" :prefix "SDL_"
                       :members (into (sorted-map) (for [n singles] [(keyword (c->clj n)) (Long/parseLong (get values n))]))}])]
    (let [p (str root "/src/sdl3/consts.clj")] (io/make-parents p) (spit p (emit-consts enum-maps define-maps)))
    ...
    (println "modules written:" (count results) "functions bound:" (- parsed (count all-skipped)) "skipped:" (count all-skipped))
    (doseq [[c why] all-skipped] (println "  skip fn" c "—" why))
    (println "consts: enums" (count enum-maps) "define groups" (count define-maps))))
